    ...
    try:
        ...
        for date in dates:
            ...
            if hour in [7, 8, 9, 17, 18, 19, 20]:
                ...
                price = BASE_PRICE * weekly_factor * (
                    1.0 + np.random.uniform(0.3, 0.5) + daily_factor)
            elif hour in [10, 11, 12, 13, 14, 15, 16]:
                # Shoulder hours with moderate variability
                price = BASE_PRICE * weekly_factor * (
                    1.0 + np.random.uniform(0.1, 0.3) + daily_factor)
            else:
                # Off-peak hours with reduced variability
                price = BASE_PRICE * weekly_factor * (
                    1.0 + np.random.uniform(-0.3, 0.0) + daily_factor)
