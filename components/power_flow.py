    ...
    try:
        ...
        def update_power_values():
            ...
            try:
                ...
                try:
                    current_pv_production = weather_service.get_pv_forecast(
                        battery.max_watt_peak, battery.pv_efficiency)
                except Exception as e:
                    logger.error(f"Error getting PV forecast: {str(e)}")
                    current_pv_production = 0.0

                if power_data:
                    return (power_data['grid_power'],
                            power_data['home_consumption'],
                            power_data['battery_power'], current_pv_production)
            except Exception:
                pass
            ...
            if 6 <= hour <= 20:  # Daylight hours
                pv_production = abs(np.sin(
                    (hour - 6) * np.pi / 14)) * (battery.max_watt_peak /
                                                 1000.0) * 0.7
            else:
                pv_production = 0.0
        ...
        battery_discharge = abs(min(0,
                                    battery_power))  # Total battery discharge
        battery_to_home = min(battery_discharge,
                              home_consumption)  # Battery power used by home
        battery_to_grid = battery_discharge - battery_to_home  # Remaining battery power to grid

        # Calculate PV flows
        pv_to_home = min(pv_production,
                         home_consumption)  # PV power used by home
        pv_to_battery = min(
            max(0, pv_production - pv_to_home),  # Remaining PV power
            battery.get_available_capacity()  # Available battery capacity
        )
        pv_to_grid = max(0, pv_production - pv_to_home -
                         pv_to_battery)  # Excess PV to grid
        ...
        home_from_grid = max(0, home_consumption - home_from_battery -
                             home_from_pv)  # Home consumption from grid
        ...
        with display_container.container():
            ...
            with cols[1]:
                ...
                if battery.max_watt_peak > 0:  # Only show if PV is configured
                    st.metric("Total PV Production ☀️",
                              f"{pv_production:.1f} kW",
                              delta="Generating"
                              if pv_production > 0.1 else "No Production")
                    # Show PV flows even when they are zero
                    st.metric(f"PV to Home {get_flow_direction(-pv_to_home)}",
                              f"{pv_to_home:.1f} kW",
                              delta="Self-consumption"
                              if pv_to_home > 0.1 else "No Flow")
                    st.metric(
            ...
            with cols[2]:
                ...
                soc_text = (f"🔋 {battery.current_soc*100:.1f}% (Min SOC)"
                            if battery.current_soc <= battery.min_soc else
                            f"🔋 {battery.current_soc*100:.1f}%")
                st.progress(value=battery.current_soc, text=soc_text)
            ...
            # Show energy balance
            total_input = grid_consumption + pv_production + (
            ...
            st.metric(get_text("energy_balance"),
                      f"{abs(energy_balance):.1f} kW",
                      delta=get_text("surplus")
                      if energy_balance > 0 else get_text("deficit"))
