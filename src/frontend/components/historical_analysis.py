...
def render_historical_analysis(battery):
    ...
    if start_date and end_date:
        if isinstance(start_date, datetime) and isinstance(
                end_date, datetime) and start_date > end_date:
        ...
        for date in dates:
            production = weather_service.get_pv_forecast(battery.max_watt_peak,
                                                         battery.pv_efficiency,
                                                         date=date)
            pv_data.append({
                'datetime': date,
                'production': float(production) / 1000,
                'date': date.date(),
                'hour': date.hour
            })
            ...
            daily_totals[date.date()] += float(production / 1000)
        ...
        fig2 = go.Figure(data=go.Heatmap(z=pivot_df.values,
                                         x=pivot_df.columns,
                                         y=pivot_df.index,
                                         colorscale='YlOrRd',
                                         name=get_text("hourly_production")))

        fig2.update_layout(title=get_text("hourly_pv_production"),
        ...
        with col1:
            st.metric(get_text("total_production"),
                      f"{total_production:.2f} kWh")

        with col2:
            st.metric(get_text("average_daily"),
                      f"{avg_daily_production:.2f} kWh/day")

        with col3:
            st.metric(get_text("peak_production"),
